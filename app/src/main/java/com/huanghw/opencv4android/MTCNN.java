...
public class MTCNN {
    ...
    private Bitmap bitmapResize(Bitmap bm, float scale){
        ...
        //CREATE A MATRIX GOR THE MANIPULATION. matrix指定图片仿射变换参数
     ...
     * (2) Foe all candidates, use NMS with threshold = 0.7
     * (3) Calibrate Bounding Box
     * 注意：CNN输入图片最上面一行，坐标为[0..width,0]。所以Bitmao需要对折后再跑网络；网络输出同理。
     */
    ...
    public Vector<Box> detectFaces(Bitmap bitmap, int minFaceSize){
        ...
        //return
        Log.e("error","[*]Mtcnn Detection Time "+(System.currentTimeMillis()-t_start));
